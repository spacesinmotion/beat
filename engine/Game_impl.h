...
#if defined(G_TEXTURE_COUNT)
  Texture textures[G_TEXTURE_COUNT];
#endif
#if defined(G_FONT_COUNT)
  FontImage fonts[G_FONT_COUNT];
#endif
...
sg_image g_image(Game *g, int img_index) {
  if (g->textures[img_index].img.id == 0)
    g->textures[img_index].img = img_load(G_TEXTURE_LIST[img_index].file);
  return g->textures[img_index].img;
}

// #if G_FONT_COUNT > 0
FontImage load_font(const FontDesc desc) {
  uint8_t *ttf_buffer = (uint8_t *)malloc(1048576);
  FontImage f = {.size = 8.0f * desc.size, .tw = 512, .th = 512};
...
const FontImage *g_font(Game *g, int font_index) {
  assert(font_index >= 0 && font_index < G_FONT_COUNT);

  if (g->fonts[font_index].texture.id == 0)
    g->fonts[font_index] = load_font(G_FONT_LIST[font_index]);

  return &g->fonts[font_index];
}

void g_create_text(Game *g, G_Text *to, G_FONT font_index, const char *text) {
